  ...
  return <main className="app-shell">
    ...
    <section className="workspace"><header className="topbar"><div><p>N K Hotels · Client Operations</p><h1>Unified Inbox</h1></div><div className="top-actions"><span className="role-badge">{role}</span><span className="connection"><i /> Connected</span><button className="ghost" onClick={logout}>Sign out</button><button className="primary" onClick={() => setContactsOpen(true)}>＋ Manage contacts</button></div></header>
      <div className={`inbox-grid mobile-${mobileView}`}>
        <section className="conversation-list"><div className="list-tools"><label className="search"><span>⌕</span><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search people, hotels or numbers" /></label><div className="filters queue-filters">{["All", "New enquiries", "Current clients", "Follow-ups", "Needs attention", "Closed"].map((item) => <button key={item} onClick={() => setFilter(item)} className={filter === item ? "active" : ""}>{item}</button>)}</div></div>
